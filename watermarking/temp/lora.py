...
def setup(
    ...
    io: IOArgs = IOArgs(
        train_data_dir=Path("data/train_watermarking"),
        val_data_dir=Path("data/train_watermarking"),
        checkpoint_dir=Path("/hpc2hdd/home/cyang662/pre-trained-models/Llama-2-70b-hf"),
        out_dir=Path("out/lora/Llama-2-70b-watermarking"),
    ),
    ...
    eval: EvalArgs = EvalArgs(interval=1000, max_new_tokens=2, max_iters=100),
) -> None:
    print(locals())
    ...
    if num_nodes == 1:
        ...
        fabric = L.Fabric(devices=devices, num_nodes=num_nodes, strategy=strategy, precision=precision, loggers=logger, plugins=plugins)
