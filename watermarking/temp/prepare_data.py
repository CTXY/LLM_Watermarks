...
def prepare(
    destination_path: Path = Path("data/train_original"),
    checkpoint_dir: Path = Path("/hpc2hdd/home/cyang662/pre-trained-models/Llama-2-7b-hf"),
    folder: str = 'train',  # train, test, both
    test_split_fraction: float = 0.03865,  # to get exactly 2000 test samples,
    seed: int = 42,
    mask_inputs: bool = False,  # as in alpaca-lora
    data_file_name: str = "train.jsonl",
    data_file_url: str = "",
    ignore_index: int = -1,
    ...
    with jsonlines.open(data_file_path, "r") as fout:
        data = []
        for line in fout:
            if line['input'] == '':
                line['input'] = None
...
def prepare_sample(example: dict, tokenizer: Tokenizer, max_length: int, mask_inputs: bool, ignore_index: int) -> dict:
    ...
    # print(encoded_full_prompt_and_response)
